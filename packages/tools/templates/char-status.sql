...
--      务必写清楚取值范围与默认行为（「注释即说明」）。
--
-- 命名空间契约：所有表名 / world_state 键必须带「包名_」前缀，
-- 加载器静态扫描强制，违规即加载失败。
-- ============================================================
...
CREATE TABLE IF NOT EXISTS PACKNAME_char_status (
  ...
  favor INTEGER NOT NULL DEFAULT 0,     -- 好感度：-100~100，初见通常为 0；正=友善，负=敌对（参考 favor 示例）
...
-- INSERT OR IGNORE：按 npc_ref 幂等，重复执行不产生重复行（seed 幂等纪律）。
